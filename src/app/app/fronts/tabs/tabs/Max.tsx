...
export default function AnalyticsMax({
  start_date,
  end_date,
}: {
  start_date: Date,
  ...
  const front_max = useMemo(() => {
    if (!fronts_in_range) return {};
    const maximums: Record<string, Duration> = {};
    ...
    for (const front of sorted_durations) {
      if (!maximums[front.member.id]) {
        maximums[front.member.id] = front.duration;
      }
    }
    return maximums;
  }, [fronts_in_range]);
  ...
  return (
    <>
      <List sx={{ width: '100%' }}>
        { Object.entries(front_max ?? {}).map(([member_id, duration]) => {
          ...
          return (
            ...
            >
              ...
              <ListItemText
                primary={member.name}
                secondary={`Fronted ${formatDuration(duration)} at most`}
              />
            </ListItem>
          )
        })}
